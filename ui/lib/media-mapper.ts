...
type RawMediaItem = {
  ...
  sizeBytes: bigint; checksum: string | null; status: string;
  createdAt: Date; updatedAt: Date;
  fileType?: { id: string; name: string; openStrategy: string } | null;
  collectionLabels?: Array<{
    collectionLabel: { id: string; collectionId: string; value: string; collection: { name: string } };
  }>;
};

export function mapMediaItem(item: RawMediaItem) {
  return {
    id: item.id,
